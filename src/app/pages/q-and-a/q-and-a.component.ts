...
export class QAndAComponent implements OnInit {
  ...
  /** Retourne q.options (si présent) ou tableau vide, sans casser la compilation si le modèle TS n'a pas "options" */
/** Retourne les options de la question */
getOptions(q: Question): string[] {
        ...
        this.reponseService.getByClientSousThematique(this.userId, stId).subscribe({
          next: existing => {
            ...
            existing.forEach(r => {
              ...
              // injecter la valeur si elle n'est pas déjà dans les options (sécurité)
  ...
  saveReponses(st: SousThematique): void {
    ...
    // Validation: toutes les questions doivent être sélectionnées
    ...
    if (manquantes.length) {
      this.showToast('Veuillez sélectionner une option pour chaque question.', 'error');
      return;
    }
